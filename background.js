...
chrome.browserAction.onClicked.addListener(function(tab) {
    ...
    chrome.browserAction.getTitle({}, function(title){
      if (title.indexOf('weizhuan0') != -1 && url.indexOf("id=levbank:miss") != -1 ) {
        chrome.browserAction.setTitle({title: 'weizhuan1'});
        chrome.browserAction.setIcon({path:'images/online.png'});
        chrome.tabs.sendMessage(tabId, {action:"online"}, function(response){});
      } else {
        chrome.browserAction.setTitle({title: 'weizhuan0'});
...
chrome.tabs.onUpdated.addListener(function(tabId, changeInfo, tab) {
  var url = tab.url;
  chrome.browserAction.getTitle({}, function(title){
  	if( title.indexOf('weizhuan1')!=-1 && url.indexOf("id=levbank:miss") != -1) {
